module TranslationFilter
  @@languages = [
     ...
     {code: "ko", name: "Korean", nativeName: "한국어 (韓國語)"},
     ...
     {code: "uz", name: "Uzbek", nativeName: "أۇزبېك‎"},
   ...
   def get_native_title code
     (@@languages.detect{|w| w[:code] == code})[:nativeName].capitalize
   end

   def get_translation key, website_data_id, lang
     if WebsiteTranslation.where({key: key, website_data_id: website_data_id}).size > 0 && lang != 'en'
       WebsiteTranslation.where({key: key, website_data_id: website_data_id}).first.value
     else
       key
     end
   end
...
Liquid::Template.register_filter(TranslationFilter)
